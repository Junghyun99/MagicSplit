    ...
    function renderStatus(data) {
        ...
        for (const [ticker, info] of Object.entries(positions)) {
            ...
            for (const lot of lots) {
                ...
                const absPctStr = Math.abs(lot.pct_change).toFixed(1) + '%';
                const ariaLabel = isPositive ? `Profit of ${absPctStr}` : `Loss of ${absPctStr}`;
                const arrowIcon = isPositive ? '▲' : '▼';

                lotsHtml += `
                    <li class="lot-item">
                        <span>${lot.buy_date} | ${lot.quantity.toLocaleString()} shares @$${lot.buy_price.toFixed(2)}</span>
                        <span class="${pctClass}" aria-label="${ariaLabel}">${pctStr} <span aria-hidden="true">${arrowIcon}</span></span>
                    </li>`;
            }
