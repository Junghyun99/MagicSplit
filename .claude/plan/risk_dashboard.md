...
### Phase 3: 리스크 점수화 및 알림 시스템 고도화 (섹터 정보 제외)
- [x] **백엔드: 리스크 지표 보강 및 점수 산출 로직 (`status_builder.py`)**
    - [x] **리스크 점수(100점 만점) 산출**:
        - **현금 비중**: 25% 미만 시 `(25 - 현재비중) * 1.5` 감점 (최대 -30점)
        - **종목 집중도**: 단일 종목 15% 초과 시 `(최대비중 - 15) * 2` 감점 (최대 -30점)
        - **고차수 비중**: Lv 8 이상 포지션 비중에 따라 `(비중 * 50)` 감점 (최대 -20점)
        - **장기 정체**: 30일 이상 무거래 종목당 4점 감점 (최대 -20점)
    - [x] **알림(Alerts) 생성**: 위 감점 항목 및 잔고 불일치 발생 시 상세 메시지 생성
    - [x] **잔고 동기화 체크**: 로컬 DB 수량과 포트폴리오 잔고 수량 전수 비교 및 `sync_error` 플래그 설정
- [x] **프론트엔드: 리스크 요약 UI 및 점수화 (`risk-view.js`)**
    - [x] 대시보드 상단에 '계좌 건강 상태(Risk Score)' 게이지 추가 (색상: 80+ 녹색, 50+ 황색, 50미만 적색)
    - [x] 집중도 차트에 15% 경고선(Threshold Line) 추가
- [x] **프론트엔드: 지능형 알림 패널 (Alert System)**
    - [x] 현재 모든 `alerts`를 리스트업하여 보여주는 알림 패널 섹션 구현
    - [x] 대시보드 메인 탭에 중대 위험 발생 시 경고 배너 노출
- [x] **데이터 구조 확장 (`risk_summary`)**
    - [x] `risk_score`, `sync_error`, `alerts` 배열을 포함하도록 확장

### Phase 4: 섹터 집중도 분석 및 시각화 고도화
- [ ] **데이터 기초 공사 (`tickers.db`)**
    - [ ] `tickers.db`에 섹터(Industry) 정보 컬럼 추가 및 데이터 수집 스크립트 작성
    - [ ] 마스터 데이터(.mst, .cod) 기반 산업 분류 매핑 테이블 구축
- [ ] **백엔드: 섹터 분석 및 리스크 점수 고도화 (`status_builder.py`)**
    - [ ] `risk_summary` 내 '섹터별 비중(sector_weights)' 산출 로직 구현
    - [ ] **섹터 집중도 리스크 점수 반영 (최대 -20점 감점)**:
        - 특정 섹터 비중 30% 초과 시 `(현재비중 - 30) * 1.0` 감점 적용
    - [ ] 섹터 과집중 시 `alerts` 메시지 생성 (예: "⚠️ [반도체] 섹터 비중 과다 (38%)")
- [ ] **프론트엔드: 섹터 시각화 및 UI 개선 (`risk-view.js`)**
    - [ ] 섹터별 비중 차트(Donut Chart 등) 구현 및 리스크 탭 추가
    - [ ] 리스크 상세 리포트 기능 (점수 감점 요인 상세 내역 표시)
