...
- [ ] **백엔드: 리스크 지표 보강 및 점수 산출 로직 (`status_builder.py`)**
    - [ ] **리스크 점수(100점 만점) 산출**:
    ...
    - [ ] **알림(Alerts) 생성**: 위 감점 항목 및 잔고 불일치 발생 시 상세 메시지 생성
    - [ ] **잔고 동기화 체크**: 로컬 DB 수량과 포트폴리오 잔고 수량 전수 비교 및 `sync_error` 플래그 설정
- [ ] **프론트엔드: 리스크 요약 UI 및 점수화 (`risk-view.js`)**
    - [ ] 대시보드 상단에 '계좌 건강 상태(Risk Score)' 게이지 추가 (색상: 80+ 녹색, 50+ 황색, 50미만 적색)
    - [ ] 집중도 차트에 15% 경고선(Threshold Line) 추가
- [ ] **프론트엔드: 지능형 알림 패널 (Alert System)**
    - [ ] 현재 모든 `alerts`를 리스트업하여 보여주는 알림 패널 섹션 구현
    - [ ] 대시보드 메인 탭에 중대 위험 발생 시 경고 배너 노출
- [ ] **데이터 구조 확장 (`risk_summary`)**
    - [ ] `risk_score`, `sync_error`, `alerts` 배열을 포함하도록 확장
