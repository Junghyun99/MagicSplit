...
- `SLACK_WEBHOOK_URL` - Slack ?�림 (공통 폴백)
- `SLACK_WEBHOOK_DOMESTIC` - 국내주식 전용 Slack 웹훅 (미설정 시 `SLACK_WEBHOOK_URL`로 폴백)
- `SLACK_WEBHOOK_OVERSEAS` - 해외주식 전용 Slack 웹훅 (미설정 시 `SLACK_WEBHOOK_URL`로 폴백)
- `SLACK_WEBHOOK_CRYPTO` - 코인 전용 Slack 웹훅 (미설정 시 `SLACK_WEBHOOK_URL`로 폴백)
- `SLACK_BOT_TOKEN` - Slack Bot OAuth 토큰 (Bot API 모드용, 공통)
- `SLACK_CHANNEL_ID` - Slack 채널 ID (공통 폴백)
- `SLACK_CHANNEL_ID_DOMESTIC` - 국내주식 전용 Slack 채널 ID (미설정 시 `SLACK_CHANNEL_ID`로 폴백)
- `SLACK_CHANNEL_ID_OVERSEAS` - 해외주식 전용 Slack 채널 ID (미설정 시 `SLACK_CHANNEL_ID`로 폴백)
- `SLACK_CHANNEL_ID_CRYPTO` - 코인 전용 Slack 채널 ID (미설정 시 `SLACK_CHANNEL_ID`로 폴백)
