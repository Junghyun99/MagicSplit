...
class JsonRepository(IRepository):
    ...
    @staticmethod
    def snapshots_from_history(history: List[dict]) -> List[dict]:
        """history.json 레코드에서 일별 스냅샷 리스트를 파생한다 (마이그레이션용).

        하루에 거래가 여러 번인 경우 save_snapshot의 같은-날짜 누적 로직과 동일하게
        반영한다:
        - portfolio_value/cash_balance/stock_value: 그날 마지막 레코드 값
        - net_deposit: 그날 모든 레코드 net_deposit의 합
          (history의 net_deposit은 직전 레코드 기준으로 계산되므로, 하루 합은
           전일 대비 순입금과 정확히 일치한다.)
        """
        # 날짜 문자열(YYYY-MM-DD[ HH:MM:SS]) 기준 안정 정렬로 시간순 보장
        ordered = sorted(
            (r for r in history if (r.get("date") or "")),
            key=lambda r: r["date"],
        )
        by_date: Dict[str, dict] = {}
        for rec in ordered:
            date_key = rec["date"][:10]
            slot = by_date.get(date_key)
            if slot is None:
                by_date[date_key] = {"last": rec, "nd": 0.0}
            else:
                slot["last"] = rec  # 그날 마지막 레코드 유지
            by_date[date_key]["nd"] += float(rec.get("net_deposit") or 0.0)

        snaps = []
        for date_key in sorted(by_date):
            rec = by_date[date_key]["last"]
            pv = float(rec.get("portfolio_value") or 0.0)
            cash = rec.get("cash_balance")
            cash_f = float(cash) if cash is not None else None
            snaps.append({
                "date": date_key,
                "portfolio_value": round(pv, 2),
                "cash_balance": round(cash_f, 2) if cash_f is not None else None,
                "stock_value": round(pv - cash_f, 2) if cash_f is not None else None,
                "net_deposit": round(by_date[date_key]["nd"], 2),
            })
        return snaps

    def migrate_snapshots_from_history(self, overwrite: bool = False) -> dict:
        """history.json으로부터 snapshots.json을 백필한다.

        overwrite=False(기본): 기존 snapshots.json에 없는 날짜만 추가(봇이 기록한
        값 우선), 날짜순 병합 저장.
        overwrite=True: history 파생값으로 전체 재작성.

        Returns: 통계 dict (history/derived/existing/written 레코드 수).
        """
        history = self._load_json(self.history_file, default=[])
        derived = self.snapshots_from_history(history)
        existing = self._load_json(self.snapshots_file, default=[])

        if overwrite:
            merged = derived
        else:
            have = {s.get("date") for s in existing}
            merged = existing + [s for s in derived if s["date"] not in have]
            merged.sort(key=lambda s: s["date"])

        self._save_json(self.snapshots_file, merged)
        return {
            "history": len(history), "derived": len(derived),
            "existing": len(existing), "written": len(merged),
        }
