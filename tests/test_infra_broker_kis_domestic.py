...
from src.infra.broker.kis_domestic import _to_kis_code
...
class TestKisDomesticBroker:
    ...
    def test_fetch_current_prices_timeout(self, mock_get):
        ...
        with patch.object(KisDomesticPaperBroker, "_auth", return_value="fake_token"):
            ...
            prices = broker.fetch_current_prices(["069500"])

            assert prices["069500"] == 100.0
...
def test_to_kis_code():
    ...
    assert _to_kis_code("000660") == "000660"
    # Padding for short input
    assert _to_kis_code("5930") == "005930"


def test_get_portfolio_returns_bare_code():
    """KIS API의 pdno(바닐라 6자리)가 그대로 holdings 키로 사용된다."""
    ...
    with patch.object(KisDomesticPaperBroker, "_auth", return_value="fake_token"), \
        ...
        broker = KisDomesticPaperBroker("key", "secret", "12345678AB", logger)
        broker.token_expires_at = datetime.now() + timedelta(hours=1)
        pf = broker.get_portfolio()

    assert "058470" in pf.holdings
    assert pf.holdings["058470"] == 10
...
class TestKisDomesticOutcomeToExecution:
    ...
    def test_partial_outcome(self):
        ...
        order = Order(ticker="005930", action=OrderAction.BUY, quantity=5, price=100.0)
    ...
    def test_rejected_outcome_zeroes_qty(self):
        ...
        order = Order(ticker="005930", action=OrderAction.SELL, quantity=5, price=100.0)
...
class TestKisDomesticQueryFillDetailsMultiRow:
    ...
    def test_multi_row_warning(self, mock_get):
        ...
        with patch.object(KisDomesticPaperBroker, "_auth", return_value="t"):
            broker = KisDomesticPaperBroker("k", "s", "12345678AB", logger)
            broker.token_expires_at = datetime.now() + timedelta(hours=1)
            price, qty, fee = broker._query_fill_details("X", "005930")
