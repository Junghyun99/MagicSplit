...
class TestSnapshotMigration:
    def test_from_history_single_per_day(self, repo):
        """하루 1거래: 그대로 일별 스냅샷으로 변환"""
        history = [
            {"date": "2026-04-01 09:00:00", "portfolio_value": 10000.0,
             "cash_balance": 10000.0, "net_deposit": 10000.0},
            {"date": "2026-04-02 09:00:00", "portfolio_value": 11000.0,
             "cash_balance": 500.0, "net_deposit": 0.0},
        ]
        snaps = JsonRepository.snapshots_from_history(history)
        assert [s["date"] for s in snaps] == ["2026-04-01", "2026-04-02"]
        assert snaps[1]["portfolio_value"] == 11000.0
        assert snaps[1]["stock_value"] == 10500.0

    def test_from_history_multi_trade_day_sums_net_deposit(self, repo):
        """하루 여러 거래: net_deposit 합산, 마지막 레코드 자산값 사용"""
        history = [
            {"date": "2026-05-11 09:38:05", "portfolio_value": 1000.0,
             "cash_balance": 800.0, "net_deposit": 1000.0},
            {"date": "2026-05-11 10:00:00", "portfolio_value": 1050.0,
             "cash_balance": 300.0, "net_deposit": 0.0},
            {"date": "2026-05-11 14:00:00", "portfolio_value": 1080.0,
             "cash_balance": 120.0, "net_deposit": 200.0},
        ]
        snaps = JsonRepository.snapshots_from_history(history)
        assert len(snaps) == 1
        s = snaps[0]
        ...
        # 마지막 레코드 값
        assert s["portfolio_value"] == 1080.0
        assert s["cash_balance"] == 120.0
        # net_deposit 합 = 1000 + 0 + 200
        assert s["net_deposit"] == 1200.0

    def test_from_history_out_of_order_sorted(self, repo):
        """레코드가 시간순이 아니어도 날짜/시간 정렬 후 마지막 값을 취한다"""
        history = [
            {"date": "2026-04-02 14:00:00", "portfolio_value": 1080.0,
             "cash_balance": 100.0, "net_deposit": 0.0},
            {"date": "2026-04-02 09:00:00", "portfolio_value": 1000.0,
             "cash_balance": 500.0, "net_deposit": 300.0},
        ]
        snaps = JsonRepository.snapshots_from_history(history)
        assert snaps[0]["portfolio_value"] == 1080.0  # 14:00 레코드가 마지막
        assert snaps[0]["net_deposit"] == 300.0

    def test_from_history_missing_net_deposit_as_zero(self, repo):
        history = [
            {"date": "2026-04-01", "portfolio_value": 1000.0, "cash_balance": 1000.0},
        ]
        snaps = JsonRepository.snapshots_from_history(history)
        assert snaps[0]["net_deposit"] == 0.0

    def test_migrate_backfill_preserves_existing(self, repo):
        """백필 모드: 기존 snapshots에 있는 날짜는 유지, 없는 날짜만 추가"""
        # 기존 스냅샷 (봇이 4/02 기록)
        repo._save_json(repo.snapshots_file, [
            {"date": "2026-04-02", "portfolio_value": 9999.0,
             "cash_balance": 1.0, "stock_value": 9998.0, "net_deposit": 5.0},
        ])
        repo._save_json(repo.history_file, [
            {"date": "2026-04-01 09:00:00", "portfolio_value": 1000.0,
             "cash_balance": 1000.0, "net_deposit": 1000.0},
            {"date": "2026-04-02 09:00:00", "portfolio_value": 1100.0,
             "cash_balance": 500.0, "net_deposit": 0.0},
        ])
        stats = repo.migrate_snapshots_from_history(overwrite=False)
        snaps = repo.load_snapshots()
        assert [s["date"] for s in snaps] == ["2026-04-01", "2026-04-02"]
        # 4/02는 기존 봇 값 유지 (덮어쓰지 않음)
        apr2 = next(s for s in snaps if s["date"] == "2026-04-02")
        assert apr2["portfolio_value"] == 9999.0
        assert stats["written"] == 2

    def test_migrate_overwrite_rebuilds(self, repo):
        """overwrite 모드: history 파생값으로 전체 재작성"""
        repo._save_json(repo.snapshots_file, [
            {"date": "2026-04-02", "portfolio_value": 9999.0,
             "cash_balance": 1.0, "stock_value": 9998.0, "net_deposit": 5.0},
        ])
        ...
        repo.migrate_snapshots_from_history(overwrite=True)
        snaps = repo.load_snapshots()
        assert len(snaps) == 1
        assert snaps[0]["portfolio_value"] == 1100.0  # 기존 9999 덮어씀
