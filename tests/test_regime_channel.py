...
class TestClassifyChannel:
    ...
    def test_band_symmetric_up_down(self):
        # 63봉간 +-13.2% 기울기: 밴드8 기준 상승/하락 대칭 판정
        df = _ohlc(_geometric_trend(63, 100.0, 0.2))
        assert classify_channel(df, lookback=63, slope_band_pct=8.0).regime == Regime.UPTREND
        df_dn = _ohlc(_geometric_trend(63, 100.0, -0.2))
        assert classify_channel(df_dn, lookback=63, slope_band_pct=8.0).regime == Regime.DOWNTREND
